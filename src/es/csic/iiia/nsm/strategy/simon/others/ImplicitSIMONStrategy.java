...
import es.csic.iiia.nsm.strategy.simon.SIMONUtilityFunction;
 ...
 */
public class ImplicitSIMONStrategy implements NormSynthesisStrategy {
	...
	protected ImplicitSIMONOperators operators;
	 ...
	 */
	public ImplicitSIMONStrategy(NormSynthesisMachine nsm,
		...
		this.operators = new ImplicitSIMONOperators(this, normReasoner, nsm);
	...
	protected void checkThresholds() {
		...
		for(Norm norm : normsToRevise) {
			...
			else if(this.normativeNetwork.isRepresented(norm) &&
					!this.normsWithGenScore.contains(norm) && 
	 ...
	 */
	protected void specialiseDown(Norm norm, List<Norm> visited) {
		...
		if(this.normativeNetwork.isRepresented(norm)) {
			...
			/* Try to generalise children that do not under-perform */
			for(Norm child : children) {
				if(this.hasGeneralisationUtility(child)) {
					this.generalisableNorms.add(child);
				}
			}

			
			/* Specialise worst children */
			boolean noChildUnderperforms = true;
			for(Norm child : children) {
				if(this.isUnderperforming(child)) {
					noChildUnderperforms= false;
					break;
				}
			}	
			
			/* TODO: Revisar esto, porque lo hemos hecho en un plis.. (JAVI) */
			/* Caso de Iosu: especializas la norma pero te das cuenta de que
			 * ningun hijo funciona mal... tienes que elegir un % de 
			 * los que peor van*/
			if(noChildUnderperforms) {
			
				float deactivationRatio = 0.1f;
				int childrenToSpecialise = (int) Math.max(1, children.size() * deactivationRatio);

				List<Norm> worstChildren = new ArrayList<Norm>();
				List<Norm> tempChildren = new ArrayList<Norm>();

				tempChildren.addAll(children);

				for(int cts = 0; cts < childrenToSpecialise; cts++) {
					Norm wc = this.getWorstChild(tempChildren);
					worstChildren.add(wc);
					tempChildren.remove(wc);
				}

				/* If the child is one of the worst, specialise */
				for(Norm child : worstChildren) {
					this.specialiseDown(child, visited);	
				}	
			}
			
			/* Caso original de SIMON */
			else {
...
//			}
	...
	/**
	 * Get the under performing child of a List of children.
	 * 
	 * @param children
	 * @return worstChild
	 */
	private Norm getWorstChild(List<Norm> children) {
		Goal g = nsm.getNormSynthesisSettings().getSystemGoals().get(0);
		float worstChildNec = Float.POSITIVE_INFINITY;
		Norm worstChild = null;

		for (Norm child : children) {
			Utility u = this.normativeNetwork.getUtility(child);
			float childNec = u.getScore(Dimension.Necessity, g);

			if (childNec <= worstChildNec) {
				worstChildNec = childNec;
				worstChild = child;
			}
		}
		return worstChild;
	 ...
	 * @return
	 */
	public ImplicitSIMONOperators getOperators() {
