 ...
 *
 */
public class XSIMONNormEvaluator {
	 ...
	 * 
	 */
	public XSIMONNormEvaluator(List<Dimension> normEvDimensions, 
			...
			NormReasoner normReasoner, XSIMONUtilityFunction utilityFunction,
			XSIMONOperators operators) {
	...
	protected NormGroup retrieveNormGroup(Norm n1, NormCompliance complN1,
		 ...
		 * it to the norm group network */
			...
			this.normGroupNetwork.activate(normGroup);
		}
		return normGroup;
	}
		...
		for(Norm norm : negRewardedNorms.keySet()) {
			...
			for(SetOfPredicatesWithTerms precond : agContexts) {
				...
				boolean exists = this.normativeNetwork.
						contains(precond, mod, action, goal);
